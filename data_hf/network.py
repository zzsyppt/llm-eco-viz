...
def generate_graph_html(eg_graph, base_model , view_type=1):
    ...
    # 初始化 PyVis 图
    net = Network(height="1000px", width="100%", notebook=True, directed=True, cdn_resources='remote')
            ...
            for model, data in sidebar_data.items():
                ...
                things_to_show_on_sidebar += f"""
                    <div style="background: #f0f0f0; padding: 15px 20px; border-radius: 5px; margin-bottom: 15px; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.1);">
                        <p style="font-size: 14px; margin: 0;"><strong>{model}</strong><br>a <span style="font-weight: bold;">a {edge_type}</span> version of <a href="https://www.huggingface.co/{father_node_zip[0]}">{father_node_zip[0]}</a></p>
...
def generate_graph():
    ...
    base_model = request.args.get('base_model', 'Qwen/Qwen2-Audio-7B-Instruct')  # 这里可以设置默认的base_model
