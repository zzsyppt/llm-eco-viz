'''
考虑的问题
1.防止网站阻塞：在爬取数据时，可能会遇到网站的反爬虫机制，导致请求失败。为了避免这种情况，可以设置重试机制，当请求失败时，进行重试。
...
# 输入 BASE MODEL 文件路径
BASE_MODEL_FILE = "basemodels_top10000likes.txt"

# 输出文件路径
OUTPUT_FOLDER = "models_output"
OUTPUT_FILE = os.path.join(OUTPUT_FOLDER, "model_tree_raw_NEW.json")
...
def fetch_models_for_category(base_model, url_template, category):
    ...
    while True:
        ...
        while retries < max_retries:
            ...
            with open("error_models.txt", "a") as f:
